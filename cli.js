...
import initSmt from './src/init-mst.js';
...
program.command('init-smt')
    .description('Initialize the SMT')
    .argument('[csv file path]', 'Path to the file that stores the cex entries')
    .action(async () => {
        // does it need to be async?
        // add path where to get the file of the entries
        await initSmt();
    });
